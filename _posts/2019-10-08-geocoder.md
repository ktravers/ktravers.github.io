...
I went back to the `geocoder` docs and recognized that `latitude` and `longitude` attributes had to live on the model I wanted to geocode (Truck). That's what the library expects, and things start to get gnarly if you try to override that. Plus after giving it more thought, there's not much to be gained by separating out these properties into a separate table. It's just as easy to drop columns as a table, should I want to deprecate this functionality in the future. And given my use case, adding a new table/model/assocations is much more code and effort than necessary. Go with the smallest about of code/effort to get the feature working; save the optimizations if/when needed down the road.
